...
> **内测功能**：单人模式下，在**横屏模式**中会显示 AI 托管调试按钮，可分别**实时开启或关闭** X / O 的 AI 托管，实现 **AI 对战 AI** 自动对局，方便~~赛博斗蛐蛐~~测试和观察 testAI 与 当前难度AI 的表现。

### 其他特性

- 联网对战使用 **WebSocket (Socket.IO)** 实时通信，支持创建/加入房间
- **断线重连**：联网对战中 **3 分钟内**断线可自动重连，房间状态与棋局进度均会保留；超时则房间过期需重新创建
- **随机棋子分配**：每局联网对战开始时，服务器随机为双方分配 X / O 棋子，并通过 **3D 翻牌过场动画**（约 5 秒）揭示各自棋子，每局重开时重新随机分配
- AI 运行于 **Web Worker** 中，避免AI计算思考时阻塞页面
- 响应式布局，适配手机与桌面端屏幕比例
- 流畅的界面切换和伪3D落子动画
